...
def colorGradientThreshold(frame, mtx, dist):
    ...
    #For Harder Challenge
    #lower_lab = np.array([182, 100, 145])
    #upper_lab = np.array([255, 255, 255])
    ...
    # For Harder Challenge Video
    #gradx = abs_sobel_thresh(gaussBlur, orient='x', thresh_min=23, thresh_max=100)
    #combined = cv.bitwise_or(gradx, np.float64(color_comb))

    combined_binary = color_comb
...
def perspectiveTransform(combined_binary): 
    # Source vertices
    vertices = [(565,470), (705,470), (1100,720), (195, 720)]

    # For Harder Challenge Video
    #vertices = [(470,515), (780,515), (1180,720), (170, 720)]
    src = np.float32(vertices)

    img_size = combined_binary.shape

    # Destination vertices
    dst = np.float32([(330,0), (img_size[1]-350,0), (img_size[1]-350,img_size[0]), (330, img_size[0])])

    # For Harder Challenge Video
    #dst = np.float32([(200,0), (img_size[1]-300,0), (img_size[1]-300,img_size[0]), (200, img_size[0])])
...
def search_around_poly(warped_binary, left_fit, right_fit):
    global prev_left_inds_poly
    global prev_right_inds_poly
    # HYPERPARAMETER
...
class Line():
    ...
    def give_best_fit(self, fitx, fit_coeff, lane_inds, binarynonzero):
        ...
        # For taking the x and y values of all the lane pixels for polynnominal fitting
        nonzero = binarynonzero.nonzero()
        nonzeroy = np.array(nonzero[0])
        nonzerox = np.array(nonzero[1])

        # If polyfit coeff was not found we use slide window again
        if fit_coeff is None:
            ...
            if self.best_fit is not None:    
                ...
                if (self.diffs[0] < 0.0003 and self.diffs[1] < 0.17 and self.diffs[2] < 80):
                    self.detected = True
                ...
                if (self.diffs[0] < 0.0003 or self.diffs[1] < 0.17 or self.diffs[2] < 80):
                    
                    if (self.diffs[0] > 0.0003 or self.diffs[1]>0.17 or self.diffs[2]>80) and len(self.current_fit)>0:
...
# Initializing object of 'VideoCapture' class to play with videos
vid = cv.VideoCapture('project_video.mp4')
frame_width = int(vid.get(3))
frame_height = int(vid.get(4))

# For saving videos
#out = cv.VideoWriter('challenge_video_output.avi', cv.VideoWriter_fourcc('M','J','P','G'), 30, (frame_width,frame_height))
...
while vid.isOpened():
    ...
    if ret==True:
        ...
        # If both the lanes are detected we use the polynoial search method
            ...
            resized = cv.resize(result, (360,240))
            cv.imshow('poly search', resized)
